...
## The mark - DONE 2026-08-12

The mark ships at **two optical sizes**. The SVGs are the source of truth;
there is no generator in the repo.

| File | Which | Used by |
| --- | --- | --- |
| `docs/assets/mark.svg` | simplified | favicon, site masthead |
| `nika_onlymap_exporter/icons/qgis2webmap.svg` | simplified (same artwork) | QGIS toolbar, Plugin Manager, plugin listing |
| `docs/assets/logo.svg` | full artwork | large use only |
| `docs/assets/banner.svg` | mark + wordmark | README header |

**Why two.** QGIS draws toolbar icons at 24 px by default. Scaled to that, the
full artwork's 0.7-unit streets land at 0.26 px and its slab gaps at 1.4 px, so
the streets dissolve, the three layers merge and the emitter disappears - a
green blob. The simplified mark drops the emitter and glow, uses two slabs, one
bold street instead of a grid and a heavier border, and holds together at 16 px.

The mark no longer follows `--accent`: it is multi-colour and carries its own
dark plate, so it reads on both themes unchanged. The masthead is now an `<img>`
pointing at the same file as the favicon, so the two cannot drift apart.

The geometry is isometric and was generated rather than drawn, so hand-edits
mean editing path data. If the mark ever changes materially, redraw it rather
than patching these files.
