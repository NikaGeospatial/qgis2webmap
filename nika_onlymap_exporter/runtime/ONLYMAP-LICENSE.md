...
**Attribution removal is a permission, not an obligation.** A valid commercial license permits Licensee to remove the Software's on-map attribution; it does not require removal. Licensee may keep the attribution displayed (`keep-badge`, or `OmMap.setKeepBadge(true)`), in which case the Software renders the credit without the Non-Commercial notice, since that notice would not describe Licensee's use.
